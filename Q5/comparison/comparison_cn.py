"""
Q5.1 四维度消融实验汇总对比（中文版）
=====================================
对比四个变量维度：降雨量 / 孔隙水压力 / 微震 / 干湿入渗系数
统一算法，一次性生成全部中文对比图表
"""
...
print('=' * 72)
print('  Q5.1 四维度消融实验汇总对比（中文版）')
print(f'  数据: {len(df)} 行, {len(all_features)} 个特征')
print('=' * 72)
...
print(f"  全特征: RMSE={rmse_full:.4f}, MAE={mae_full:.4f}, R2={r2_full:.4f}")

results = {}
for abl_key, abl_def in ablation_defs.items():
    print(f"\n--- 训练(去掉{abl_def['name_cn']}) ---")
    keywords = abl_def['keywords']
    ablated_features = [f for f in all_features 
                        if not any(kw in f for kw in keywords)]
    print(f"  特征数: {len(ablated_features)} (移除 {len(all_features) - len(ablated_features)} 项)")
...
ax.set_ylabel('Δ RMSE (毫米)')
ax.set_title(f'总体 RMSE 变化\n(基准 RMSE = {rmse_full:.4f} 毫米)', fontsize=11, fontweight='bold')
...
ax.set_xticklabels(abl_labels, fontsize=10)
ax.set_ylabel('Δ R²')
ax.set_title(f'总体 R² 变化\n(基准 R² = {r2_full:.4f})', fontsize=11, fontweight='bold')
ax.grid(axis='y', alpha=0.3)
...
bars_full = ax.bar(x_pos - width/2, [rmse_full]*len(abl_keys), width, 
                    label=f'全特征 ({len(all_features)} 项)', color='#34495e', edgecolor='white')
bars_ab = ax.bar(x_pos + width/2, [results[k]['rmse_ablated'] for k in abl_keys], width,
                 label='消融后', color='#e74c3c', edgecolor='white')
ax.set_xticks(x_pos)
ax.set_xticklabels(abl_labels, fontsize=9)
ax.set_ylabel('RMSE (毫米)')
ax.set_title('RMSE：全特征 vs 消融', fontsize=11, fontweight='bold')
...
ax.set_xticklabels(abl_labels, fontsize=9)
ax.set_ylabel('Δ RMSE (毫米)')
ax.set_title(f'缓慢变形阶段\n(基准 RMSE = {phase_full[0]["rmse"]:.4f})', fontsize=10, fontweight='bold')
ax.grid(axis='y', alpha=0.3)
...
ax.set_xticklabels(abl_labels, fontsize=9)
ax.set_ylabel('Δ RMSE (毫米)')
ax.set_title(f'加速变形阶段  ⚠\n(基准 RMSE = {phase_full[1]["rmse"]:.4f})', fontsize=10, fontweight='bold')
ax.grid(axis='y', alpha=0.3)
...
ax.set_xticklabels(abl_labels, fontsize=9)
ax.set_ylabel('Δ RMSE (毫米)')
ax.set_title(f'快速变形阶段\n(基准 RMSE = {phase_full[2]["rmse"]:.4f})', fontsize=10, fontweight='bold')
ax.grid(axis='y', alpha=0.3)

fig.suptitle('Q5.1 四维度消融实验汇总对比\n'
             f'全特征模型：{len(all_features)} 项特征，RMSE={rmse_full:.4f}，MAE={mae_full:.4f}，R²={r2_full:.4f}',
             fontsize=13, fontweight='bold', y=0.99)
fig.tight_layout(rect=[0, 0, 1, 0.96])
fig.savefig(os.path.join(script_dir, 'comparison_overview.png'), dpi=150, bbox_inches='tight')
plt.close(fig)
print(f"\n  [图片] comparison_overview.png 已保存（中文版）")
...
col_labels = ['维度', '全特征 RMSE', '消融后 RMSE', 'Δ RMSE', '增幅 %',
              '缓慢阶段 Δ', '加速阶段 Δ', '快速阶段 Δ', '全特征 R²', '消融后 R²', 'Δ R²']
table_data = []
for k in abl_keys:
    r = results[k]
    d = ablation_defs[k]
...
ax.set_title('Q5.1 消融实验汇总表\n'
             f'全特征模型：{len(all_features)} 项特征 | 样本数：{len(df)} | '
             f'阶段划分：缓慢(0-{b1-1}) 加速({b1}-{b2-1}) 快速({b2}-{len(df)-1})',
             fontsize=11, fontweight='bold', pad=20)

fig2.tight_layout()
fig2.savefig(os.path.join(script_dir, 'comparison_table.png'), dpi=150, bbox_inches='tight')
plt.close(fig2)
print(f"  [图片] comparison_table.png 已保存（中文版）")

# ============================================================
# 图表 3: RMSE 增幅排序 + 阶段热力 [全中文]
# ============================================================
...
ax.set_xlabel('Δ RMSE (毫米)')
ax.set_title('消融后 RMSE 增幅排序\n(按影响程度排序)', fontsize=12, fontweight='bold')
ax.invert_yaxis()
ax.grid(axis='x', alpha=0.3)

# 右: 阶段热力矩阵
ax = axes3[1]
phase_names = ['缓慢变形', '加速变形', '快速变形']
...
cbar.set_label('Δ RMSE (毫米)', fontsize=10)
ax.set_title('分阶段 RMSE 变化热力图\n'
             f'(基准: 缓慢={phase_full[0]["rmse"]:.4f} | '
             f'加速={phase_full[1]["rmse"]:.4f} | 快速={phase_full[2]["rmse"]:.4f})',
             fontsize=11, fontweight='bold')

fig3.suptitle('Q5.1 消融影响排序与阶段敏感性',
              fontsize=13, fontweight='bold', y=0.99)
fig3.tight_layout(rect=[0, 0, 1, 0.96])
fig3.savefig(os.path.join(script_dir, 'comparison_ranking.png'), dpi=150, bbox_inches='tight')
plt.close(fig3)
print(f"  [图片] comparison_ranking.png 已保存（中文版）")
...
print("  最终汇总")
print("=" * 72)
print(f"\n基准 (全特征模型): RMSE={rmse_full:.4f}, MAE={mae_full:.4f}, R2={r2_full:.4f}")
...
print(f"{'(基准)':>12}")
for phase_id, phase_name in [(0, '缓慢变形'), (1, '加速变形'), (2, '快速变形')]:
    print(f"{phase_name:<8}", end='')
    bl_rmse = phase_full[phase_id]['rmse']
    for k in abl_keys:
        delta = results[k]['phase_deltas'].get(phase_id, 0)
        print(f"{delta:>+12.4f}", end=' ')
    print(f"({bl_rmse:.4f})")

print(f"\n  [完成] 所有中文图表已保存至 {script_dir}")
