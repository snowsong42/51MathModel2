"""
问题 4.5：分阶段多元线性回归 —— 表面速度建模（仅训练集）
================================================================
读入 4.4 生成的 ap4_features.xlsx 的 train sheet，
按 Stage 列分组，对每个阶段用训练集拟合 速度 ~ R_eff + P_drive + M_cum + BlastMem，
输出：
  1) model_params.csv —— 三阶段模型参数与评价指标（含链式预测所需信息）
  2) fit_evaluation.png —— 总览：三阶段 位移 对比（对齐 MATLAB 样式）
  3) segN/stage_SD_diagnosis.png —— 各阶段位移（位移）诊断报告（2×2）
  4) segN/stage_delta_diagnosis.png —— 各阶段速度（速度）诊断报告（2×2）
  5) segN/stage_coefficients.png —— 各阶段标准化特征系数柱状图
  6) 控制台 —— 全中文格式化报告（位移 残差 + 速度 残差双重指标）
...
for stage_id in [1, 2, 3]:
    ...
    # --- 位移残差（位移 累积层面）---
    sd_residual = df_train['SD'].values - df_train['SD_pred'].values
    r2_sd  = r2_score(df_train['SD'].values, df_train['SD_pred'].values)
    mae_sd = mean_absolute_error(df_train['SD'].values, df_train['SD_pred'].values)
    rmse_sd = np.sqrt(mean_squared_error(df_train['SD'].values, df_train['SD_pred'].values))
    max_res_sd = np.max(np.abs(sd_residual))
    ...
    params = {
        ...
        'coef_C_event_norm': model.coef_[2],
    ...
    for j, name in enumerate(feature_names):
        ...
        print(f"    {name:<20s} : {sign}{model.coef_[j]:.6f}  (每 +1σ 对 速度 的贡献)")
    ...
    print("  【模型性能】")
    print(f"  ┌─ 位移残差:")
    print(f"  │   决定系数 R²           : {r2_sd:.6f}  ({r2_sd*100:.2f}%)")
    print(f"  │   平均绝对误差 MAE       : {mae_sd:.4f} mm")
    print(f"  │   均方根误差 RMSE        : {rmse_sd:.4f} mm")
    print(f"  │   最大 |残差|            : {max_res_sd:.4f} mm")
    print(f"  └─")
    print(f"  ┌─ 速度残差:")
    print(f"  │   决定系数 R²           : {r2:.4f}  ({r2*100:.1f}%)")
    print(f"  │   平均绝对误差 MAE       : {mae:.6f} mm/min")
    print(f"  │   均方根误差 RMSE        : {rmse:.6f} mm/min")
    print(f"  └─")
    ...
    for j, name in enumerate(feature_names):
        coef = model.coef_[j]
        if coef > 0:
            print(f"    • {name}：系数为正（{coef:+.6f}），每 +1σ 使 速度 增大约 {abs(coef):.4f} mm/min，"
                  f"是滑坡的\"促进因子\"。")
        else:
            print(f"    • {name}：系数为负（{coef:+.6f}），每 +1σ 使 速度 减小约 {abs(coef):.4f} mm/min，"
                  f"可能起\"抑制/滞后\"作用。")
...
cols_order = ['Stage', 'Stage Name', 'Intercept',
              'coef_R_eff_norm', 'coef_P_drive_norm', 'coef_C_event_norm', 'coef_BlastMem_norm',
              'R2', 'MAE', 'RMSE', 'MSE',
...
for i in range(3):
    ...
    r2_delta   = results[i]['R2']
    mae_delta  = results[i]['MAE']
    rmse_delta = results[i]['RMSE']
    r2_sd_i    = r2_score(sd_true, sd_pred)
    mae_sd_i   = mean_absolute_error(sd_true, sd_pred)
    rmse_sd_i  = np.sqrt(mean_squared_error(sd_true, sd_pred))
    coefs = [results[i]['coef_R_eff_norm'], results[i]['coef_P_drive_norm'],
             results[i]['coef_C_event_norm'], results[i]['coef_BlastMem_norm']]

    line_color = stage_line_rgb[i]

    # ---------------------------------------------------------------
    # 图1：位移诊断报告（SD 层面）→ stage_SD_diagnosis.png
    # 2×2 布局：
    #  ① SD 实际 vs 拟合 时序对比
    #  ② SD 预测值 vs 实际值 散点
    #  ③ SD 残差 时序图
    ...
    fig_sd.suptitle(f'{stage_names[i]} · 位移诊断报告',
    ...
    ax1.set_title('① 表面位移位移 · 时序对比', fontsize=12, fontweight='bold')
    ax1.set_xlabel('时间序号')
    ax1.set_ylabel('表面位移 (mm)')
    ax1.legend(fontsize=9, loc='upper left')
    ax1.grid(alpha=0.3)
    ax1.text(0.98, 0.05, f'R² = {r2_sd_i:.6f}', transform=ax1.transAxes,
    ...
    ax2.set_xlabel('实际位移 (mm)')
    ax2.set_ylabel('拟合位移 (mm)')
    ax2.set_title('② 预测位移 vs 实际位移', fontsize=12, fontweight='bold')
    ax2.legend(fontsize=9)
    ax2.grid(alpha=0.25)
    ax2.set_aspect('equal')
    ax2.text(0.05, 0.95, f'R² = {r2_sd_i:.6f}\nMAE = {mae_sd_i:.4f} mm',
    ...
    std_sd_res = np.std(sd_res)
    ax3.axhline(+2 * std_sd_res, color='red', linestyle=':', linewidth=1.0, alpha=0.7, label='+2σ')
    ax3.axhline(-2 * std_sd_res, color='red', linestyle=':', linewidth=1.0, alpha=0.7, label='-2σ')
    outlier_sd_ratio = np.mean(np.abs(sd_res) > 2 * std_sd_res) * 100
    ax3.set_title(f'③ 位移残差时序（超出 ±2σ: {outlier_sd_ratio:.1f}%）',
    ...
    mu_sd, sigma_sd = stats.norm.fit(sd_res)
    x_norm_sd = np.linspace(sd_res.min(), sd_res.max(), 200)
    ax4.plot(x_norm_sd, stats.norm.pdf(x_norm_sd, mu_sd, sigma_sd), 'r-', linewidth=1.8,
             label=f'N({mu_sd:.4f}, {sigma_sd:.4f}²)')
    ax4.axvline(0, color='black', linestyle='--', linewidth=1.0)
    ax4.set_title('④ 位移残差分布', fontsize=12, fontweight='bold')
    ax4.set_xlabel('位移残差 (mm)')
    ax4.set_ylabel('概率密度')
    ax4.legend(fontsize=9)
    ax4.text(0.98, 0.95, f'均值 = {mu_sd:.4f}\n标准差 = {sigma_sd:.4f}',
             transform=ax4.transAxes, fontsize=8, verticalalignment='top',
             horizontalalignment='right',
             bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.85))

    sd_save_path = os.path.join(seg_dirs[i], "stage_SD_diagnosis.png")
    fig_sd.savefig(sd_save_path, dpi=200, bbox_inches='tight')
    plt.close(fig_sd)
    print(f"[OK] {stage_names[i]} · 位移诊断报告 → {sd_save_path}")

    # ---------------------------------------------------------------
    # 图2：速度诊断报告（速度 层面）→ stage_delta_diagnosis.png
    # 2×2 布局：
    #  ① 速度 实际 vs 预测 时序对比
    #  ② 速度 预测值 vs 实际值 散点
    #  ③ 速度 残差 时序图
    ...
    fig_delta.suptitle(f'{stage_names[i]} · 速度诊断报告',
                       fontsize=14, fontweight='bold')
    ...
    ax1.plot(t, y_true, 'k-', linewidth=0.9, alpha=0.7, label='实际速度')
    ax1.plot(t, y_pred, color=line_color, linewidth=2, linestyle='--', label='预测速度')
    ax1.set_title('① 速度速度 · 时序对比', fontsize=12, fontweight='bold')
    ax1.set_xlabel('时间序号')
    ax1.set_ylabel('速度 (mm/min)')
    ax1.legend(fontsize=9, loc='upper left')
    ax1.grid(alpha=0.3)
    ax1.text(0.98, 0.95, f'R² = {r2_delta:.4f}', transform=ax1.transAxes,
             fontsize=10, verticalalignment='top', horizontalalignment='right',
             bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.85))

    # ② 速度 预测 vs 实际 散点
    ax2 = fig_delta.add_subplot(gs_d[0, 1])
    ax2.scatter(y_true, y_pred, c=[line_color], alpha=0.45, s=18, edgecolors='none')
    lim_min = min(y_true.min(), y_pred.min())
    lim_max = max(y_true.max(), y_pred.max())
    margin = (lim_max - lim_min) * 0.08
    ax2.plot([lim_min - margin, lim_max + margin], [lim_min - margin, lim_max + margin],
             'k--', linewidth=1.0, alpha=0.7, label='y = x')
    ax2.set_xlabel('实际速度 (mm/min)')
    ax2.set_ylabel('预测速度 (mm/min)')
    ax2.set_title('② 预测速度 vs 实际速度', fontsize=12, fontweight='bold')
    ax2.legend(fontsize=9)
    ax2.grid(alpha=0.25)
    ax2.set_aspect('equal')
    ax2.text(0.05, 0.95, f'R² = {r2_delta:.4f}\nMAE = {mae_delta:.4f} mm/min',
             transform=ax2.transAxes, fontsize=9, verticalalignment='top',
             bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.85))
    ...
    ax3.axhline(0, color='black', linestyle='--', linewidth=0.8)
    std_delta_res = np.std(delta_residual)
    ax3.axhline(+2 * std_delta_res, color='red', linestyle=':', linewidth=1.0, alpha=0.7, label='+2σ')
    ax3.axhline(-2 * std_delta_res, color='red', linestyle=':', linewidth=1.0, alpha=0.7, label='-2σ')
    outlier_delta_ratio = np.mean(np.abs(delta_residual) > 2 * std_delta_res) * 100
    ax3.set_title(f'③ 速度残差时序（超出 ±2σ: {outlier_delta_ratio:.1f}%）',
                  fontsize=12, fontweight='bold')
    ax3.set_xlabel('时间序号')
    ax3.set_ylabel('速度残差 (mm/min)')
    ax3.legend(fontsize=8, loc='upper right')
    ax3.grid(alpha=0.25)
    ...
    ax4.plot(x_norm_d, stats.norm.pdf(x_norm_d, mu_d, sigma_d), 'r-', linewidth=1.8,
             label=f'N({mu_d:.4f}, {sigma_d:.4f}²)')
    ax4.axvline(0, color='black', linestyle='--', linewidth=1.0)
    ax4.set_title('④ 速度残差分布', fontsize=12, fontweight='bold')
    ax4.set_xlabel('速度残差 (mm/min)')
    ax4.set_ylabel('概率密度')
    ax4.legend(fontsize=9)
    ...
    delta_save_path = os.path.join(seg_dirs[i], "stage_delta_diagnosis.png")
    fig_delta.savefig(delta_save_path, dpi=200, bbox_inches='tight')
    plt.close(fig_delta)
    print(f"[OK] {stage_names[i]} · 速度诊断报告 → {delta_save_path}")

    # ---------------------------------------------------------------
    # 图3：标准化特征系数柱状图 → stage_coefficients.png
    # ---------------------------------------------------------------
    fig_coef = plt.figure(figsize=(8, 5))
    fig_coef.suptitle(f'{stage_names[i]} · 标准化特征系数 (β_std)',
                      fontsize=13, fontweight='bold')
    ax_coef = fig_coef.add_subplot(111)
    colors_bar = ['#28a745' if c >= 0 else '#dc3545' for c in coefs]
    bars = ax_coef.barh(feature_names, coefs, color=colors_bar, edgecolor='white', height=0.6)
    ax_coef.axvline(0, color='black', linewidth=0.8)
    ax_coef.set_xlabel('β_std (mm·min⁻¹·σ⁻¹)', fontsize=11)
    for bar, val in zip(bars, coefs):
        offset = 0.001 if abs(val) < 0.0005 else val * 0.05
        ax_coef.text(val + offset, bar.get_y() + bar.get_height()/2,
                     f'{val:+.6f}', va='center', fontsize=10,
                     color='#155724' if val >= 0 else '#721c24')
    ax_coef.grid(alpha=0.25, axis='x')

    coef_save_path = os.path.join(seg_dirs[i], "stage_coefficients.png")
    fig_coef.savefig(coef_save_path, dpi=200, bbox_inches='tight')
    plt.close(fig_coef)
    print(f"[OK] {stage_names[i]} · 系数柱状图 → {coef_save_path}")
...
for i in range(3):
    ...
    print(f"    ├─ 位移诊断      → {seg_dirs[i]}/stage_SD_diagnosis.png")
    print(f"    ├─ 速度诊断      → {seg_dirs[i]}/stage_delta_diagnosis.png")
    print(f"    └─ 系数柱状图    → {seg_dirs[i]}/stage_coefficients.png")
print()
