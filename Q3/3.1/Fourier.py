...
# 2. 读取降噪后数据
df_den = pd.read_excel(os.path.join(script_dir, "train_denoised.xlsx"))
den_col_map = {
    'a': 'a: Rainfall (mm)',
    'b': 'b: Pore Water Pressure (kPa)',
    'c': 'c: Microseismic Event Count',
...
post_denoise = {}
for key in ['a','b','c','d','e']:
    post_denoise[key] = df_den[den_col_map[key]].values.astype(float)

# 3. 包络函数（采用 Pchip 保持单调不振荡，强制包含端点）
def get_envelope(f, Pxx, mode='upper'):
    ...
    pks = np.interp(locs, f[peaks], Pxx[peaks])      # 用线性插值获取端点幅值
