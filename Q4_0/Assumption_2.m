%% ============================================================
% 混合分段检测 + 固定分点（从 Assumption_1 读取）自适应多项式拟合模型
%  前半段用 'std' 检测 1 拐点
%  固定拐点使用 Assumption_1 的第 1 个分段点
%  后半段用 'linear' 检测 2 拐点
...
v = dataV.Velocity_mm_min_raw;            % 原始速度（仅 sgolay，未混合滤波）

N = length(x_fact);
dt = 10;
t_min = (t_idx - 1) * dt;

%% ---- 0.5 读取 Assumption_1 的输出参数 ----
paramTable = readtable('Assumption1_Params.xlsx');
split_idx = paramTable.FirstSplitIndex;          % 固定分点索引
split_t_fixed = paramTable.FirstSplitTime_min;   % 固定分点时间(min)

fprintf('从 Assumption_1 读取固定分点: 索引 %d, 时间 %.1f min\n', split_idx, split_t_fixed);

%% ---- 1. 混合分段点检测 (0~split_t_fixed用std, >split_t_fixed用linear) ----

% 前半段：用 std 统计量检测 1 个拐点
...
% 各段颜色名称（对应 segColors 着色方案）
segColorNames = {'绿色', '橙色', '红色', '红色', '蓝色'};
...
for i = 1:nSeg
    ...
    % 输出拟合公式（含颜色和天数换算）
    d_start = seg_t(1) / 1440;
    d_end   = seg_t(end) / 1440;
    fprintf('\n阶段 %d(t = %.1f~%.1f min = %.2f~%.2f 天，点 %d~%d, 阶数 %d):\n', ...
        i, seg_t(1), seg_t(end), d_start, d_end, idx_start, idx_end, ord);
    if ord == 1
        fprintf('  v(t) = %.6f + %.6e * t\n', p(2), p(1));
...
% 在颜色切换处标注日期（天数），同色内部拐点不标（置于最上层覆盖虚线）
h_texts = [];
for i = 1:nSeg-1
    if i <= length(segColorNames) && ~strcmp(segColorNames{i}, segColorNames{i+1})
        cp_boundary = segments(i, 2);
        t_cp = t_min(cp_boundary);
        d_cp = t_cp / 1440;
        ht = text(t_cp, yl(2) - 0.05*(yl(2)-yl(1)), ...
            sprintf('%.2f天', d_cp), ...
...
yl1 = ylim;
for j = 1:nSeg-1
    if j <= length(segColorNames) && ~strcmp(segColorNames{j}, segColorNames{j+1})
        t_cp = t_min(segments(j, 2));
        d_cp = t_cp / 1440;
        ht = text(t_cp, yl1(2) - 0.05*(yl1(2)-yl1(1)), ...
            sprintf('%.2f天', d_cp), ...
...
if ~isempty(hts1), uistack(hts1, 'top'); end
ylabel('速度 (mm/min)');
...
for j = 1:nSeg-1
    if j <= length(segColorNames) && ~strcmp(segColorNames{j}, segColorNames{j+1})
        t_cp = t_min(segments(j, 2));
        d_cp = t_cp / 1440;
        ht = text(t_cp, yl2(2) - 0.05*(yl2(2)-yl2(1)), ...
            sprintf('%.2f天', d_cp), ...
            'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', ...
            'FontSize', 8, 'BackgroundColor', 'w', 'EdgeColor', 'k');
        hts2 = [hts2; ht];
...
if ~isempty(hts2), uistack(hts2, 'top'); end
ylabel('位移 (mm)');
...
for j = 1:nSeg-1
    if j <= length(segColorNames) && ~strcmp(segColorNames{j}, segColorNames{j+1})
        t_cp = t_min(segments(j, 2));
        d_cp = t_cp / 1440;
        ht = text(t_cp, yl3(2) - 0.05*(yl3(2)-yl3(1)), ...
            sprintf('%.2f天', d_cp), ...
            'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', ...
            'FontSize', 8, 'BackgroundColor', 'w', 'EdgeColor', 'k');
        hts3 = [hts3; ht];
...
if ~isempty(hts3), uistack(hts3, 'top'); end
xlabel('时间 t (min)');
ylabel('位移残差 (mm)');
title('位移拟合残差（实测 - 模型）');
grid on;

sgtitle(sprintf('混合分段模型验证（std + linear）'));
