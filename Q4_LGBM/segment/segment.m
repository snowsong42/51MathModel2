%% ============================================================
% Q4 位移分段检测与拟合脚本（MATLAB 版）
% 适配自 Q5/segment/segment.m
%
% 合并：Filter.m + Differentiate.m + Assumption_3.m
% 输入：..\feature\train_features.xlsx（取 Displacement 列）
% 输出：segment.csv（含断点索引、阶段统计）
...
FEATURE_FILE = fullfile(SCRIPT_DIR, '..', 'common', 'Attachment 4.xlsx');
OUT_DIR = SCRIPT_DIR;

fprintf('============================================================\n');
fprintf('Q4 位移分段检测与拟合（MATLAB）\n');
fprintf('============================================================\n');
...
if any(strcmpi(colNames, 'Surface Displacement (mm)'))
    x_raw = dataTable.('Surface Displacement (mm)');
else
    % 尝试其他可能的列名
...
plot(t_min, x_raw, 'b-', 'LineWidth', 0.5); hold on;
plot(t_min, x_filt, 'k-', 'LineWidth', 1.5);
title('④ 原始 vs 最终滤波结果'); xlabel('Serial No.'); ylabel('Displacement (mm)');
legend('原始','最终','Location','best'); grid on;

%% ---- 2.5 导出滤波后位移 ----
fprintf('\n[2.5/6] 导出滤波后位移到 displacement_filtered.csv...\n');
% 用 dataTable.Time 作为时间列
time_col = dataTable.Time;  % datetime 类型
out_table = table(time_col, x_filt(:), 'VariableNames', {'Time', 'Displacement_filtered'});
csv_out_path = fullfile(OUT_DIR, 'displacement_filtered.csv');
writetable(out_table, csv_out_path);
fprintf('  已保存: %s (%d 条记录)\n', csv_out_path, height(out_table));
