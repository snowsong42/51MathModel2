...
Q5.1 微震 消融对比实验
====================
对比：保留 Microseismic 全部特征 vs 去掉 Microseismic 及其衍生特征
生成两张效果对比图
"""
...
print('=' * 72)
print('  Q5.1 Microseismic Ablation Experiment')
print('=' * 72)
...
# 去掉 Microseismic 及其衍生特征
micro_related = ['Microseismic', 'microseismic']
no_micro_features = [f for f in all_features 
                     if not any(kw in f for kw in micro_related)]

df = df.dropna(subset=all_features + ['Delta_D']).reset_index(drop=True)

print(f"\nFeatures WITH Microseismic:     {len(all_features)} features")
print(f"Features WITHOUT Microseismic:  {len(no_micro_features)} features")
...
# 训练两个模型
# ============================================================
print("\n--- Training WITH Microseismic (全变量) ---")
y_pred_with, metrics_with, rmse_with, mae_with, r2_with = train_phase_model(df, all_features)

print("\n--- Training WITHOUT Microseismic (去掉微震) ---")
y_pred_without, metrics_without, rmse_without, mae_without, r2_without = train_phase_model(df, no_micro_features)

# ============================================================
# 打印结果
# ============================================================
print("\n" + "=" * 72)
print("  RESULTS: Microseismic Ablation Comparison")
print("=" * 72)
print(f"\n{'Metric':<15} {'WITH Microseismic':>20} {'WITHOUT Microseismic':>23} {'Delta':>12}")
print("-" * 72)
...
ax1.plot(df.index, disp_cum_with, 'b-', alpha=0.6, linewidth=0.8, 
         label=f'With Microseismic (RMSE={rmse_with:.4f}, R2={r2_with:.3f})')
ax1.plot(df.index, disp_cum_without, 'r--', alpha=0.7, linewidth=0.8, 
         label=f'Without Microseismic (RMSE={rmse_without:.4f}, R2={r2_without:.3f})')
...
ax1.set_title(f'Microseismic Ablation: Cumulative Displacement Prediction\n'
              f'RMSE with Microseismic={rmse_with:.4f} -> without={rmse_without:.4f}  '
...
ax2.plot(df.index, y_pred_with, 'b-', alpha=0.6, linewidth=0.5, label='With Microseismic')
ax2.plot(df.index, y_pred_without, 'r--', alpha=0.6, linewidth=0.5, label='Without Microseismic')
for bx, color in [(b1, 'green'), (b2, 'orange')]:
...
ax3.plot(df.index, resid_with, 'b-', alpha=0.4, linewidth=0.4, 
         label=f'With Microseismic (std={np.std(resid_with):.4f})')
ax3.plot(df.index, resid_without, 'r--', alpha=0.4, linewidth=0.4, 
         label=f'Without Microseismic (std={np.std(resid_without):.4f})')
...
fig.tight_layout()
fig.savefig(os.path.join(output_dir, 'microseismic_ablation_timeseries.png'), dpi=150)
plt.close(fig)
print(f"\n  [Chart] microseismic_ablation_timeseries.png saved")
...
ax_left.set_title(f'With Microseismic ({len(all_features)} features)\n'
...
ax_right.set_title(f'Without Microseismic ({len(no_micro_features)} features)\n'
                   f'RMSE={rmse_without:.4f}, MAE={mae_without:.4f}, R2={r2_without:.4f}')

fig2.suptitle(f'Microseismic Ablation: Scatter Comparison\n'
              f'RMSE change: {rmse_with:.4f} -> {rmse_without:.4f} '
              f'(Δ={(rmse_without - rmse_with):.4f} mm)', fontsize=12, fontweight='bold')
fig2.tight_layout()
fig2.savefig(os.path.join(output_dir, 'microseismic_ablation_scatter.png'), dpi=150)
plt.close(fig2)
print(f"  [Chart] microseismic_ablation_scatter.png saved")
