...
Q5.1 孔隙水压力 消融对比实验
============================
对比：保留 PorePressure 全部特征 vs 去掉 PorePressure 及其衍生特征
生成两张效果对比图
"""
...
os.chdir(script_dir)
output_dir = script_dir  # 图表直接输出到当前文件夹
...
# 数据清洗
print('=' * 72)
print('  Q5.1 PorePressure Ablation Experiment')
print('=' * 72)
...
# 去掉 PorePressure 及其衍生特征
pore_related = ['PorePressure', 'Pore_', 'Pore_Diff', 'Pore_Rain_Cross', 'Pore_Infiltration_Cross']
no_pore_features = [f for f in all_features 
                    if not any(kw in f for kw in pore_related)]

df = df.dropna(subset=all_features + ['Delta_D']).reset_index(drop=True)

print(f"\nFeatures WITH PorePressure:     {len(all_features)} features")
print(f"Features WITHOUT PorePressure:  {len(no_pore_features)} features")
...
# 训练两个模型
# ============================================================
print("\n--- Training WITH PorePressure (全变量) ---")
y_pred_with, metrics_with, rmse_with, mae_with, r2_with = train_phase_model(df, all_features)

print("\n--- Training WITHOUT PorePressure (去掉孔隙水压力) ---")
y_pred_without, metrics_without, rmse_without, mae_without, r2_without = train_phase_model(df, no_pore_features)

# ============================================================
# 打印结果
# ============================================================
print("\n" + "=" * 72)
print("  RESULTS: PorePressure Ablation Comparison")
print("=" * 72)
print(f"\n{'Metric':<15} {'WITH PorePressure':>20} {'WITHOUT PorePressure':>23} {'Delta':>12}")
print("-" * 72)
print(f"{'RMSE (mm)':<15} {rmse_with:>20.4f} {rmse_without:>23.4f} {rmse_without - rmse_with:>+12.4f}")
print(f"{'MAE (mm)':<15} {mae_with:>20.4f} {mae_without:>23.4f} {mae_without - mae_with:>+12.4f}")
print(f"{'R2':<15} {r2_with:>20.4f} {r2_without:>23.4f} {r2_without - r2_with:>+12.4f}")
...
# ============================================================
# 图表 1: 累积位移预测对比 (with vs without PorePressure)
# ============================================================
...
ax1.plot(df.index, disp_cum_with, 'b-', alpha=0.6, linewidth=0.8, 
         label=f'With PorePressure (RMSE={rmse_with:.4f}, R2={r2_with:.3f})')
ax1.plot(df.index, disp_cum_without, 'r--', alpha=0.7, linewidth=0.8, 
         label=f'Without PorePressure (RMSE={rmse_without:.4f}, R2={r2_without:.3f})')
...
ax1.set_title(f'PorePressure Ablation: Cumulative Displacement Prediction\n'
              f'RMSE with PorePressure={rmse_with:.4f} -> without={rmse_without:.4f}  '
...
ax2.plot(df.index, y_pred_with, 'b-', alpha=0.6, linewidth=0.5, label='With PorePressure')
ax2.plot(df.index, y_pred_without, 'r--', alpha=0.6, linewidth=0.5, label='Without PorePressure')
...
ax3.plot(df.index, resid_with, 'b-', alpha=0.4, linewidth=0.4, 
         label=f'With PorePressure (std={np.std(resid_with):.4f})')
ax3.plot(df.index, resid_without, 'r--', alpha=0.4, linewidth=0.4, 
         label=f'Without PorePressure (std={np.std(resid_without):.4f})')
...
fig.tight_layout()
fig.savefig(os.path.join(output_dir, 'pore_pressure_ablation_timeseries.png'), dpi=150)
plt.close(fig)
print(f"\n  [Chart] pore_pressure_ablation_timeseries.png saved")

# ============================================================
# 图表 2: 散点密度对比 (with vs without PorePressure)
# ============================================================
...
ax_left.set_title(f'With PorePressure ({len(all_features)} features)\n'
...
ax_right.set_title(f'Without PorePressure ({len(no_pore_features)} features)\n'
                   f'RMSE={rmse_without:.4f}, MAE={mae_without:.4f}, R2={r2_without:.4f}')

fig2.suptitle(f'PorePressure Ablation: Scatter Comparison\n'
              f'RMSE change: {rmse_with:.4f} -> {rmse_without:.4f} '
              f'(Δ={(rmse_without - rmse_with):.4f} mm)', fontsize=12, fontweight='bold')
fig2.tight_layout()
fig2.savefig(os.path.join(output_dir, 'pore_pressure_ablation_scatter.png'), dpi=150)
plt.close(fig2)
print(f"  [Chart] pore_pressure_ablation_scatter.png saved")
