"""
Q5.1 Rainfall 消融对比实验
==========================
对比：保留 Rainfall 全部特征 vs 去掉 Rainfall 及其衍生特征
生成两张效果对比图
"""
...
os.chdir(script_dir)
output_dir = os.path.join(script_dir, 'rainfall_ablation_charts')
...
# 数据清洗
print('=' * 72)
print('  Q5.1 Rainfall Ablation Experiment')
print('=' * 72)
...
# 去掉 Rainfall 的特征列表
rainfall_related = ['Rain_f', 'Rain_', 'Rainfall']
no_rain_features = [f for f in all_features 
                    if not any(kw in f for kw in rainfall_related)]
# 也去掉 Pore_Rain_Cross
no_rain_features = [f for f in no_rain_features if f != 'Pore_Rain_Cross']

df = df.dropna(subset=all_features + ['Delta_D']).reset_index(drop=True)

print(f"\nFeatures WITH Rainfall:     {len(all_features)} features")
print(f"Features WITHOUT Rainfall:  {len(no_rain_features)} features")
...
# 训练两个模型
# ============================================================
print("\n--- Training WITH Rainfall ---")
y_pred_with, metrics_with, rmse_with, mae_with, r2_with = train_phase_model(df, all_features)

print("\n--- Training WITHOUT Rainfall ---")
y_pred_without, metrics_without, rmse_without, mae_without, r2_without = train_phase_model(df, no_rain_features)

# ============================================================
# 打印结果
# ============================================================
print("\n" + "=" * 72)
print("  RESULTS: Rainfall Ablation Comparison")
print("=" * 72)
print(f"\n{'Metric':<15} {'WITH Rainfall':>15} {'WITHOUT Rainfall':>17} {'Delta':>12}")
print("-" * 62)
print(f"{'RMSE (mm)':<15} {rmse_with:>15.4f} {rmse_without:>17.4f} {rmse_without - rmse_with:>+12.4f}")
print(f"{'MAE (mm)':<15} {mae_with:>15.4f} {mae_without:>17.4f} {mae_without - mae_with:>+12.4f}")
print(f"{'R2':<15} {r2_with:>15.4f} {r2_without:>17.4f} {r2_without - r2_with:>+12.4f}")
...
ax1.plot(df.index, disp_cum_with, 'b-', alpha=0.6, linewidth=0.8, label=f'With Rainfall (RMSE={rmse_with:.4f}, R2={r2_with:.3f})')
ax1.plot(df.index, disp_cum_without, 'r--', alpha=0.7, linewidth=0.8, label=f'Without Rainfall (RMSE={rmse_without:.4f}, R2={r2_without:.3f})')

for bx, color, lbl in [(b1, 'green', 'Slow'), (b2, 'orange', 'Accel')]:
    ax1.axvline(x=bx, color=color, linestyle='--', alpha=0.6, linewidth=1)
ax1.legend(loc='upper left', fontsize=9)
ax1.set_ylabel('Cumulative Displacement (mm)')
ax1.set_title(f'Rainfall Ablation: Cumulative Displacement Prediction\n'
              f'RMSE with={rmse_with:.4f} -> without={rmse_without:.4f}  '
              f'(+{(rmse_without - rmse_with):.4f} mm, +{(rmse_without/rmse_with - 1)*100:.1f}%)')
...
ax2.plot(df.index, y_pred_with, 'b-', alpha=0.6, linewidth=0.5, label='With Rainfall')
ax2.plot(df.index, y_pred_without, 'r--', alpha=0.6, linewidth=0.5, label='Without Rainfall')
...
ax3.plot(df.index, resid_with, 'b-', alpha=0.4, linewidth=0.4, label=f'With Rainfall (std={np.std(resid_with):.4f})')
ax3.plot(df.index, resid_without, 'r--', alpha=0.4, linewidth=0.4, label=f'Without Rainfall (std={np.std(resid_without):.4f})')
...
fig.tight_layout()
fig.savefig(os.path.join(output_dir, 'rainfall_ablation_timeseries.png'), dpi=150)
plt.close(fig)
print(f"\n  [Chart] rainfall_ablation_timeseries.png saved")

# ============================================================
# 图表 2: 散点密度对比 (with vs without)
# ============================================================
...
ax_left.set_title(f'With Rainfall ({len(all_features)} features)\n'
...
ax_right.set_title(f'Without Rainfall ({len(no_rain_features)} features)\n'
                   f'RMSE={rmse_without:.4f}, MAE={mae_without:.4f}, R2={r2_without:.4f}')

fig2.suptitle(f'Rainfall Ablation: Scatter Comparison\n'
              f'RMSE change: {rmse_with:.4f} -> {rmse_without:.4f} '
              f'(+{(rmse_without - rmse_with):.4f} mm)', fontsize=12, fontweight='bold')
fig2.tight_layout()
fig2.savefig(os.path.join(output_dir, 'rainfall_ablation_scatter.png'), dpi=150)
plt.close(fig2)
print(f"  [Chart] rainfall_ablation_scatter.png saved")
