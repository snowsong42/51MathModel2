...
segBgColors = {[0.7 1 0.7], [1 1 0.7], [1 0.7 0.7], [1 0.7 0.7]};
segNames = {'第1段', '第2段', '第3段', '第4段'};
% 线条分段着色用颜色（第4部分使用）
segColors = {[0 0.6 0], [0.85 0.33 0.1], [0.8 0 0], [0.8 0 0], [0 0.3 0.8]};

figure('Position', [100, 100, 1000, 500]);
hold on;

% 先绘制位移曲线（统一用黑色）
plot(t_min, x_fact, 'k-', 'LineWidth', 1.2, 'DisplayName', '滤波后位移');
yl = ylim;
...
ylabel('速度 (mm/min)');
title('速度：平滑测量值 vs 混合分段多项式拟合');
legend('平滑速度', '拟合速度（按段着色）', '拐点', 'Location', 'best');
grid on;
...
for i = 1:nSeg
    ...
    idx_end   = segments(i, 2);
    if idx_start > idx_end, continue; end
    plot(t_min(idx_start:idx_end), x_fact(idx_start:idx_end), '-', ...
        'Color', segColors{min(i,5)}, 'LineWidth', 1.2);
...
ylabel('位移 (mm)');
title('位移：滤波测量值 vs 模型积分位移');
legend('滤波后位移', '拟合位移', 'Location', 'best');
grid on;
...
sgtitle(sprintf('混合分段模型验证（rms + 固定点 %d min + linear，共 %d 段）', split_t_fixed, nSeg));
