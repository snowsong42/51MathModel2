...
v = dataV.Velocity_mm_min_filtered;        % 平滑速度 (mm/min)
...
subplot(3,1,1);
plot(t_min, v, 'k.', 'MarkerSize', 3); hold on;
plot(t_min, v_fit, 'r-', 'LineWidth', 1.5);
for i = 1:length(cp_idx)
    xline(cp_time(i), '--', 'LineWidth', 1.0);
end
ylabel('速度 (mm/min)');
title('速度：平滑测量值 vs 四段线性拟合（匀加速）');
legend('平滑速度', '拟合速度 (分段直线)', '拐点', 'Location', 'best');
grid on;
...
ylabel('位移 (mm)');
title('位移：滤波测量值 vs 由分段匀加速度积分得到的位移');
legend('滤波后位移', '拟合位移 (四段匀加速)', '拐点', 'Location', 'best');
grid on;
